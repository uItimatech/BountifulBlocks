...
public class MudPotBlockEntityRenderer implements BlockEntityRenderer<MudPotBlockEntity> {
    ...
    private static final Identifier BASE_TEXTURE = Identifier.of(BountifulBlocks.MOD_ID,"block/mud_pot_base");
    private static final Identifier SIDE_TEXTURE = Identifier.of(BountifulBlocks.MOD_ID,"block/mud_pot_side");
    ...
    public void render(MudPotBlockEntity mudPotBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
        ...
        Sherds sherds = mudPotBlockEntity.getSherds();
        this.renderDecoratedSide(this.front, matrixStack, vertexConsumerProvider, i, j, MudPotBlockEntityRenderer.getTextureIdFromSherd(sherds.front().isPresent()? sherds.front().get() : ItemStack.EMPTY.getItem()));
        this.renderDecoratedSide(this.back, matrixStack, vertexConsumerProvider, i, j, MudPotBlockEntityRenderer.getTextureIdFromSherd(sherds.back().isPresent()? sherds.back().get() : ItemStack.EMPTY.getItem()));
        this.renderDecoratedSide(this.left, matrixStack, vertexConsumerProvider, i, j, MudPotBlockEntityRenderer.getTextureIdFromSherd(sherds.left().isPresent()? sherds.left().get() : ItemStack.EMPTY.getItem()));
        this.renderDecoratedSide(this.right, matrixStack, vertexConsumerProvider, i, j, MudPotBlockEntityRenderer.getTextureIdFromSherd(sherds.right().isPresent()? sherds.right().get() : ItemStack.EMPTY.getItem()));
        matrixStack.pop();
    }
